...
export type ScanType="WEBSITE"|"APP_STORE"|"PLAY_STORE";
export interface ScanInput{scanId:string;projectId:string;url:string;scanType:ScanType;maxPages:number;debugMode:boolean;riskyFonts:string[]}
export interface TypographyRecord{text:string;tag:string;xpath:string;selector:string;fontFamily:string;fontSize:string;fontWeight:string;fontStyle:string;lineHeight:string;letterSpacing:string;textTransform:string;color:string;fontStretch:string;fontVariationSettings:string;fallback:boolean;visibility:string;display:string;riskLevel:RiskLevel;matchedRiskyFonts:string[];bounds:{x:number;y:number;width:number;height:number}}
...
export interface StoreListing{platform:"Apple App Store"|"Google Play";name:string;developer:string;description:string;rating:string;version:string;iconUrl:string;imageUrls:string[];metadata:Record<string,string>}
