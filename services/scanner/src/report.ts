...
function validate(input: unknown): Report {
  ...
  if (
    ...
    !Array.isArray(report.riskFonts)
  )
    throw new Error("Invalid report data");
  if (report.pages.length > 10_000)
    throw new Error("Report exceeds the supported page limit");
  return report;
}

export async function generateReport(input: unknown) {
  const report = validate(input),
    definitions = new Map<number, Array<Uint8Array>>(),
    pageIds: number[] = [];
  let next = 4;
  ...
  for (const [index, page] of report.pages.entries()) {
    ...
    if (page.riskCount) {
      ...
      if (image) addImagePage(image, `RISK EVIDENCE - ${page.title}`, page.url);
    }
  }
  for(const[index,asset]of(report.storeAssets||[]).filter(asset=>asset.kind==="SCREENSHOT").slice(0,8).entries()){
    const image=await loadImage(asset.url);if(image)addImagePage(image,`APP PREVIEW ${index+1} - VISUAL FONT REVIEW`,report.url);
  }
